...
void sim_thread_pulse(const shared_ptr<ch_frb_io::intensity_network_ostream> &ostream, double num_seconds, beamParams *beam, int thread_id)
{
  ...
  */
  ...
  class sim_pulse sim0(100,20,100,1000);
	...
	long iteration = 0;				    
  //std::cout<<"nchunks "<<nchunks<<" block_nchunks "<<block_nchunks<<"\n";
...
void sim_thread_file(const shared_ptr<ch_frb_io::intensity_network_ostream> &ostream, double num_seconds, beamParams *beam, int thread_id)
{
  ifstream fpin;
  fpin.open("simpulse.dat",ios_base::binary);
  ...
  class sim_pulse sim0(beam[thread_id*4].width,beam[thread_id*4].snr,100,beam[thread_id*4].dm);
	class sim_pulse sim1(beam[thread_id*4+1].width,beam[thread_id*4+1].snr,100,beam[thread_id*4+1].dm);
	class sim_pulse sim2(beam[thread_id*4+2].width,beam[thread_id*4+2].snr,100,beam[thread_id*4+2].dm);
	class sim_pulse sim3(beam[thread_id*4+3].width,beam[thread_id*4+3].snr,100,beam[thread_id*4+3].dm);
    ...
    float location = iteration*200+100;
		...
		if(ichunk%block_nchunks==0)
		{
			...
			for(int i=0;i<4;i++)
			{
				...
				if(i==0) sim0.reinitialize(beam[thread_id*4].width,beam[thread_id*4].snr,location,beam[thread_id*4].dm);
        if(i==1) sim1.reinitialize(beam[thread_id*4+1].width,beam[thread_id*4+1].snr,location,beam[thread_id*4+1].dm);
				if(i==2) sim2.reinitialize(beam[thread_id*4+2].width,beam[thread_id*4+2].snr,location,beam[thread_id*4+2].dm);
				if(i==3) sim3.reinitialize(beam[thread_id*4+3].width,beam[thread_id*4+3].snr,location,beam[thread_id*4+3].dm);

			  std::cout<<"Beam: "<<thread_id*4+i<<" SNR: "<<beam[thread_id*4+i].snr<<" Width: "<<3*beam[thread_id*4+i].width<<" DM: "<<beam[thread_id*4+i].dm<<" Time: "<<location<<std::endl;
     }
     iteration += 1;
		}
...
int main(int argc, char **argv)
{
    ...
    for (int ithread = 0; ithread < p.nthreads_tot; ithread++)
		{	
	    if(file_flag) threads[ithread] = std::thread(sim_thread_file, streams[ithread], num_seconds, beam, ithread); 
